...
public sealed class AuthService
{
    ...
    public async Task<AuthResponse> EmailSignInAsync(EmailSignInRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<AuthResponse> SocialSignInAsync(SocialSignInRequest request, CancellationToken cancellationToken)
    {
        var store = await _dataStore.LoadAsync(cancellationToken);
        var email = request.Email.Trim().ToLowerInvariant();
        var user = store.Users.FirstOrDefault(candidate =>
            candidate.Email == email ||
            (candidate.Provider == request.Provider && candidate.ProviderUserId == request.ProviderUserId));
        ...
        if (user is null)
        {
            ...
            {
                Email = email,
                Name = string.IsNullOrWhiteSpace(request.Name) ? request.ProviderUserId : request.Name.Trim(),
    ...
    public async Task<User?> GetUserFromTokenAsync(string? authHeader, CancellationToken cancellationToken)
    {
        ...
        return session is null ? null : store.Users.FirstOrDefault(candidate => candidate.Id == session.UserId);
    }
}