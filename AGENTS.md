...
- `apps/api/tests`: current executable .NET test harness that exercises MVP behaviors. Treat this as legacy placement, not the preferred future layout.
...
- Backend checks:
  - current harness: `dotnet run --project apps/api/tests/Langoose.Api.Tests.csproj --configfile D:\Projects\langoose\apps\api\NuGet.Config`
- Backend build:
...
- For study-flow changes, review both [StudyService.cs](D:\Projects\langoose\apps\api\Services\StudyService.cs) and the executable tests in [Program.cs](D:\Projects\langoose\apps\api\tests\Program.cs).
