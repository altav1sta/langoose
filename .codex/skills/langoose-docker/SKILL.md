...
- Keep Dockerfiles environment-agnostic. Put local-dev connection strings, hostnames, ports, and other environment-specific runtime values in Compose or external env configuration, not in the image itself.
...
- Follow the persistence approach currently present in the repo. If the API uses PostgreSQL, wire containers around the database service and verify the actual connection path instead of preserving older JSON-store assumptions.
...
- If live verification fails, debug the running container logs first and fix the real startup issue before declaring the Docker work complete.
- If a Docker verification step fails, is blocked by the environment, or does not complete, do not describe it as passing from memory or inference. Call out the gap, rerun it if possible, and only report success after the real check succeeds.
